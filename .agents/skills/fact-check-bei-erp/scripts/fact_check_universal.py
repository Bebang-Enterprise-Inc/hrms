	...
	for table in doc.tables:
		...
		for ridx, row in enumerate(table.rows, start=1):
			...
			if vals:
				text = " | ".join(vals)
				chunks.append(
					SourceChunk(source=path.name, locator=f"table {tidx} row {ridx}", text=text[:2200])
				)
...
def render_report(
	...
	flagged = [
		r
		for r in results
		if r["result"]["verdict"] in {"CONTRADICTED", "NOT_FOUND", "PARTIAL", "INSUFFICIENT_CONTEXT", "ERROR"}
	]
	if flagged:
		...
		for item in flagged:
			...
			if res.get("citations"):
				...
				for c in res["citations"]:
					lines.append(
						f"- `{c.get('source', '')}` @ `{c.get('locator', '')}`: {c.get('evidence', '')}"
					)
			if res.get("checks"):
...
def main() -> None:
	...
	parser.add_argument(
		"--mode", choices=["auto", "decisions", "document", "contract", "dataset"], default="auto"
	)
	...
	if workers == 1:
		...
		def _run_one(i: int) -> tuple[int, dict]:
			...
			with lock:
				...
				n = completed[0]
			print(
				f"  [{n}/{len(claims)}] {claim.claim_id}: {claim.text[:55]}... -> {res['verdict']} ({res.get('confidence', 0):.2f})"
			)
