...
def load_sources(source_dir):
	...
	for f in source_path.glob("*.md"):
		...
		if (
			"NLI_AUDIT" in f.name
			or "manual_verification" in f.name
			or "GLM_AUDIT" in f.name
			or "CONSOLIDATED" in f.name
		):
			continue
		try:
...
def verify_decision(api_key, decision, relevant_sources, model):
	...
	claim_text = (
		f"{decision.get('decision', '')} {decision.get('value', '')} {decision.get('confirmed_by', '')}"
	)
...
def main():
	...
	if workers == 1:
		...
		for i, decision in enumerate(decisions):
			print(
				f"  [{i+1}/{len(decisions)}] {decision['id']}: {decision['decision'][:60]}...",
				end="",
				flush=True,
			)
			...
			if verdict in ("ERROR", "PARSE_ERROR"):
				...
				print(
					f"\n  FAILED — {len(errors_found)} error(s). Fix GLM-5 issues before retrying.",
					file=sys.stderr,
				)
				sys.exit(10)
			time.sleep(1.2)
		...
		def process_decision(idx):
			if abort_flag.is_set():
				return idx, {
					"verdict": "SKIPPED",
					"confidence": 0,
					"explanation": "Aborted due to prior error",
					"issues": [],
			...
			if verdict in ("ERROR", "PARSE_ERROR"):
				abort_flag.set()
			print(
				f"  [{n}/{len(decisions)}] {decision['id']}: {decision['decision'][:40]}... -> {verdict} ({result['confidence']:.2f})"
			)
			return idx, result
