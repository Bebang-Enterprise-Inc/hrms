...
## Governor-Managed Deployment (Default)

For the BEI-ERP repo (`Bebang-Enterprise-Inc/hrms`), deployment is managed by `bei-governor`:
- **Builder agents create PRs** targeting `production` -- that's all
- **Governor handles:** AI review, merge serialization, deploy trigger (`build-and-deploy.yml`), L1 smoke
- **Builder agents resume:** L2-L4 verification after governor's L1 passes

**Do NOT** call `gh workflow run build-and-deploy.yml` directly.
**Do NOT** merge PRs manually with `gh pr merge`.
**Do NOT** use SSM commands to deploy unless explicitly told to for emergency bypass.

The methods below are reference documentation and emergency-bypass procedures.
...
|-----------------|--------|------|
| Python/API changes | Full rebuild via GitHub Actions | ~1 min build + ~1 min deploy |
| Python/API changes (no cache) | Full rebuild with `no_cache=true` | ~1 min build + ~1 min deploy |
| DocType schema changes | Rebuild + `bench migrate` | ~3 min total |
...
- Without cache: ~54 seconds avg (guaranteed fresh code, uses Containerfile.fast overlay build)
...
| Docker Image | `ghcr.io/bebang-enterprise-inc/bebang-erpnext-hrms:latest` (GHCR) / `samkarazi/bebang-erpnext-hrms:v15` (Docker Hub baseline) |
| Site | `hq.bebang.ph` |
...
## Emergency / Manual Methods (Governor Bypass)

Use these methods ONLY when bei-governor is down or the user explicitly requests manual deployment.
...
# 3. Deploy via PR (PR creation only -- bei-governor handles merge and triggers deploy)
/pr-deploy
```

This creates a PR targeting `production`. The `bei-governor` handles merge serialization and triggers the build-and-deploy workflow with appropriate flags.
...
aws ssm send-command \
  ...
  --parameters 'commands=[
    ...
    "docker pull ghcr.io/bebang-enterprise-inc/bebang-erpnext-hrms:latest",
    "docker service update --image ghcr.io/bebang-enterprise-inc/bebang-erpnext-hrms:latest frappe_backend",
    "docker service update --image ghcr.io/bebang-enterprise-inc/bebang-erpnext-hrms:latest frappe_frontend",
    "docker service update --image ghcr.io/bebang-enterprise-inc/bebang-erpnext-hrms:latest frappe_websocket",
    "docker service update --image ghcr.io/bebang-enterprise-inc/bebang-erpnext-hrms:latest frappe_queue-short",
    "docker service update --image ghcr.io/bebang-enterprise-inc/bebang-erpnext-hrms:latest frappe_queue-long",
    "docker service update --image ghcr.io/bebang-enterprise-inc/bebang-erpnext-hrms:latest frappe_scheduler",
    "docker service ls",
    "echo === Cleaning up old images (keeping 4 newest) ===",
    "docker container prune -f",
    "IMAGES=$(docker images ghcr.io/bebang-enterprise-inc/bebang-erpnext-hrms --format \"{{.ID}}\" | tail -n +5)",
...
# 4. Push to GHCR (primary registry)
echo $GITHUB_TOKEN | docker login ghcr.io -u USERNAME --password-stdin
docker push ghcr.io/bebang-enterprise-inc/bebang-erpnext-hrms:v15-$(date +%Y%m%d)
docker push ghcr.io/bebang-enterprise-inc/bebang-erpnext-hrms:latest
...
When using the governor pipeline, L1 smoke is governor-owned. Agents start post-deploy verification at L2.
...
```bash
# Find previous tag from GHCR or deployment logs
...
aws ssm send-command \
  ...
  --document-name "AWS-RunShellScript" \
  --parameters 'commands=[
    "docker service update --image ghcr.io/bebang-enterprise-inc/bebang-erpnext-hrms:PREVIOUS_SHA frappe_backend",
    "docker service update --image ghcr.io/bebang-enterprise-inc/bebang-erpnext-hrms:PREVIOUS_SHA frappe_frontend",
    "docker service update --image ghcr.io/bebang-enterprise-inc/bebang-erpnext-hrms:PREVIOUS_SHA frappe_websocket",
    "docker service update --image ghcr.io/bebang-enterprise-inc/bebang-erpnext-hrms:PREVIOUS_SHA frappe_queue-short",
    "docker service update --image ghcr.io/bebang-enterprise-inc/bebang-erpnext-hrms:PREVIOUS_SHA frappe_queue-long",
    "docker service update --image ghcr.io/bebang-enterprise-inc/bebang-erpnext-hrms:PREVIOUS_SHA frappe_scheduler"
  ]' \
  --output json
```
...
|--------|-------------|--------------|
| `GITHUB_TOKEN` | Auto-provided per workflow run | GitHub Actions built-in (GHCR auth) |
...
**Symptom:** Build completes suspiciously fast (~30s instead of ~54s), but new APIs/pages return "module has no attribute" or 404 errors.
...
|------------|---------------|-----------------|
| ~30s | CACHED - Old code deployed | Rebuild with `no_cache=true` |
| ~54s | FRESH - New code deployed (Containerfile.fast) | None - working correctly |
...
| File | Purpose |
|------|---------|
| `.github/docker/Containerfile.fast` | Fast overlay Containerfile (~54s builds) |
