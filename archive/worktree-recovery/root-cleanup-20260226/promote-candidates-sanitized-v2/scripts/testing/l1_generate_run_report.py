#!/usr/bin/env python3
"""
Generate markdown report from an l1_api_check_runner JSON output file.
"""
...
def render_report(run_file: Path, payload: dict[str, Any]) -> str:
    ...
    lines: list[str] = []
    lines.append("# L1 API Check Report")
    lines.append("")
    ...
    lines.append("")
    lines.append("| Module | Total | PASS | WARN | FAIL |")
    lines.append("|---|---:|---:|---:|---:|")
    for module in sorted(by_module.keys()):
        row = by_module[module]
        lines.append(
            f"| {module} | {row.get('total', 0)} | {row.get('passed', 0)} | {row.get('warned', 0)} | {row.get('failed', 0)} |"
        )
    ...
    if not fails:
        ...
        for row in fails:
            lines.append(
                f"- `{row.get('module')}` `{row.get('endpoint')}` status={row.get('status_code')} role={row.get('role')} detail={row.get('detail')}"
            )
    ...
    if not warns:
        ...
        for row in warns:
            lines.append(
                f"- `{row.get('module')}` `{row.get('endpoint')}` status={row.get('status_code')} role={row.get('role')}"
            )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Generate L1 markdown report from run JSON.")
    parser.add_argument("--run-file", required=True, help="Path to output/l1/runs/l1_run_*.json")
    parser.add_argument(
        "--out",
        default="",
        help="Optional output path. Default: docs/testing/reports/l1_run_<id>.md",
    )
    ...
    out_file = Path(args.out) if args.out else DEFAULT_REPORT_DIR / f"l1_run_{run_id}.md"
