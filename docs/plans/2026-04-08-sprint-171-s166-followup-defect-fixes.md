...
branch: s171-s166-followup-defect-fixes
status: GO
...
frontend_pr: null
backend_pr: null
sprint_registry_row: "S171 reserved on 2026-04-08. Branch: s171-s166-followup-defect-fixes. Parent: origin/production (post #497 merge)."
---

# S171: S166 Follow-up Defect Fixes
...
The strict 2026-04-08 audit caught a fabrication: R3's retest summary claimed Defect #6 was CLOSED but the underlying evidence file said `STILL_BROKEN`. PR #496 corrected the registry, PR #497 added the audit-gate rule to `/l3-v2-bei-erp`. S171 now actually fixes the bug.
...
S171 fixes the real product bugs in this list and re-runs the L3 scenarios that those bugs blocked.
...
- **Defect #16 root cause is fragile** — `_activate_compensation_change` lives at `payroll_compensation.py:1054` (S171 will need to inspect lines 1054-1100 to find the try/except). The R5 audit (S166) reported the swallow at lines 633-639 — that may have been line numbers from an earlier file version. The execution agent must grep for `_activate_compensation_change` and read the actual function body before assuming line numbers.
...
- [ ] Branch compliance: am I on `s171-s166-followup-defect-fixes` (created from `origin/production`)?
...
3. **Create sprint branch:** `git fetch origin production && git checkout -b s171-s166-followup-defect-fixes origin/production`. Same in bei-tasks: `cd ../bei-tasks && git fetch origin main && git checkout -b s171-s166-followup-defect-fixes origin/main`. NEVER write code on production/main.
...
**Verification:**
- Both repos on `s171-s166-followup-defect-fixes` branch
...
```python
"""
S171 — Backfill SSAs for BCCs that reached Approved without creating an SSA.
...
**Scope: scenarios from S166 that should now pass post-S171 deploy.**
...
After Sam confirms S171 is deployed, dispatch a fresh subagent (NOT the orchestrator session) with this brief:

> Re-run the ~25 unblocked scenarios listed in `docs/plans/2026-04-08-sprint-171-s166-followup-defect-fixes.md` Phase 8. Use real browser via Playwright headless. Use proven patterns from `scripts/testing/l3_s166_lane_a_phase2_runner.mjs`. For each scenario, write evidence to `output/s171/retest/{lane}/evidence/{scenario_id}-retest.json` with `actions: [...]`, `screenshots`, `network: [...]`. Cleanup any test data created in finally via `/frappe-bulk-edits`. STOP after writing files. Do NOT touch git. Do NOT mark scenarios PASS without browser proof.
...
- Update `output/l3/s166/DEFECTS.csv` row to `CLOSED (S171 verified)` with evidence pointer
- Note: `DEFECTS.csv` is the canonical S166 registry — write the S171 retest verification INTO that file as the audit trail
...
Change S171 row status PLANNED → COMPLETED with date + PR refs. `git add -f` since `docs/` is gitignored.

**Task 9.3 — Create PRs** (1 unit)

Two PRs (one per repo):
```bash
GH_TOKEN="" gh pr create --repo Bebang-Enterprise-Inc/hrms --base production --head s171-s166-followup-defect-fixes --title "S171: S166 follow-up defect fixes (Phases 1-7 + retest)"  --body "..."
GH_TOKEN="" gh pr create --repo Bebang-Enterprise-Inc/BEI-Tasks --base main --head s171-s166-followup-defect-fixes --title "S171: S166 follow-up defect fixes (frontend half)" --body "..."
```
...
- **completion_condition:**
  ...
  - SPRINT_REGISTRY.md S171 row updated
...
- **canonical_closeout_artifacts:**
  ...
  - `output/l3/s166/DEFECTS.csv` (updated with S171 verification)
  - `docs/plans/2026-04-08-sprint-171-s166-followup-defect-fixes.md` (this file, status=COMPLETED)
...
| Compensation page interaction | `scripts/testing/l3_s166_lane_a_phase4_runner.mjs` (reference — Lane A4 used the API path; agent must adapt to use real browser for S171) |
