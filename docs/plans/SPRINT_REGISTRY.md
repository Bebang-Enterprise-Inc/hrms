...
| `S171` | Sprint 171 | `s171-s166-followup-defect-fixes` | — | PLANNED 2026-04-08 — Follow-up defect-fix sprint addressing the 12-14 product defects still OPEN after S166 closeout + S170 partial deploy + 2026-04-08 audit (PR #496). Phase 1: Defects #6+#21 compensation Edit button + list-page modal (chicken-and-egg) — REAL bug confirmed via direct retest. Phase 2: Defect #16 _activate_compensation_change silent failure (try/except swallows). Phase 3: Defect #19 OT/apply RoleGuard crew exclusion. Phase 4: Defect #18 BEI Incident Report.store Link Warehouse but UI passes Branch (4 disciplinary scenarios blocked). Phase 5: Defect #8 create_employee_direct cached employee_id constant. Phase 6: Defect #13 emergency_phone_number silently dropped on Employee save. Phase 7: Smaller defects (#5 Generate Slips, #9 proxy 403, #11, #15, #20 attendance prereq doc). Phase 8: L3 retest of S166 SKIP scenarios that S171 unblocks (TERMINATE chain post-Phase 4, SALARY chain post-Phase 1+2, OT post-Phase 3, EMP-UX-004 list-modal post-Phase 1). Phase 9: Closeout. Estimated 65-75 units. Cold-start ready with file:line refs. Includes audit gates per S166 lesson. | `docs/plans/2026-04-08-sprint-171-s166-followup-defect-fixes.md` |
