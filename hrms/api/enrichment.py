...
def submit_edit_request(
	...
	requested_value: str,
	reason: str,
	government_id_photo: str | None = None,
) -> dict:
	...
	if field_name not in HR_APPROVAL_FIELDS:
		frappe.throw(
			_("Field '{0}' does not require HR approval. Use self-service update.").format(field_name)
		)
...
@frappe.whitelist()
def get_pending_edit_requests(branch: str | None = None, field_name: str | None = None) -> list:
...
@frappe.whitelist()
def search_employees(query: str, branch: str | None = None, limit: int = 20) -> list:
...
def send_enrichment_reminders(
	...
	for emp in targets:
		try:
			...
			failures.append(f"{emp['name']}: {e!s}")
			failed_count += 1
...
def bulk_import_gov_ids(csv_content=None, csv_file_url=None):
	...
	if required_header not in (reader.fieldnames or []):
		frappe.throw(
			_("CSV must have 'employee_id' column. Found: {0}").format(", ".join(reader.fieldnames or []))
		)
