...
def _build_default_test_fixture() -> (
...
def build_risk_snapshot_row(source: dict, horizon_hours: int = 72) -> dict:
	...
	inbound_po_qty = max(
		0.0, flt(source.get("inbound_po_qty") or source.get("open_po_qty") or source.get("inbound_qty") or 0)
	)
...
def _build_pipeline_details_from_source_rows(item_code: str, warehouse: str, source_rows: list[dict]) -> dict:
	...
	for idx, source in enumerate(source_rows, start=1):
		...
		is_pending_fallback = bool(
			not raw_source_name or not raw_expected_eta or not raw_link_route or not raw_supplier
		)
			...
			delayed_deliveries.append(
				{
					...
					"is_synthetic_fallback": bool(
						is_pending_fallback or not raw_delivery_id or not raw_delay_reason
					),
					"source_doctype": source_doctype,
					"source_name": source_name,
					"link_route": link_route,
...
def _build_pipeline_details_from_row(row: dict) -> dict:
	...
	if source_rows:
		return _build_pipeline_details_from_source_rows(
			item_code=item_code, warehouse=warehouse, source_rows=source_rows
		)
