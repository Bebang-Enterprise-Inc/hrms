...
def _classify_employee_policy(employee_row: dict[str, Any]) -> dict[str, Any]:
    ...
    if is_area_supervisor:
        ...
        cohort = "store_oic"
    elif is_office_like and is_supervisor_grade:
...
def _get_requested_field_keys_from_request(doc_or_row: Any) -> list[str]:
    ...
    requested_changes = _as_dict(getattr(doc_or_row, "requested_changes", None) or doc_or_row.get("requested_changes"))
...
def _check_request_stale(req: Any) -> tuple[bool, dict[str, Any]]:
    ...
    employee_doc = frappe.db.get_value(
        "Employee",
        req.employee,
        list({INPUT_TO_EMPLOYEE_FIELD_MAP.get(key, key) for key in snapshot.keys() if INPUT_TO_EMPLOYEE_FIELD_MAP.get(key, key)}),
        as_dict=True,
    ) or {}
    ...
    for request_key, snapshot_value in snapshot.items():
        ...
        if _norm_text(current_value) != _norm_text(snapshot_value) and _norm_text(current_value) != _norm_text(requested_value):
            stale_fields.append(
                {
                    "field": request_key,
...
def submit_enrichment_update(
    ...
    changes = _normalize_requested_changes(_as_dict(requested_changes))
    ...
    for row in open_requests:
        ...
        existing_doc.approver_notes = "\n".join(
            part for part in [existing_doc.approver_notes or "", "Superseded by a newer enrichment submission."] if part
        )
...
def approve_and_apply(
    ...
    if approved_fields:
        try:
            parsed_fields = json.loads(approved_fields) if isinstance(approved_fields, str) else approved_fields
            if isinstance(parsed_fields, list) and parsed_fields:
                approved_field_set = {str(field) for field in parsed_fields if str(field) in requested_field_keys}
        except Exception:
            approved_field_set = set(requested_field_keys)
    ...
    if req.request_type == "update_existing" and req.employee:
        try:
            employee_before = frappe.db.get_value(
                "Employee",
                req.employee,
                [
                    "first_name",
            ...
            if updates:
                ...
                if any(key in updates for key in ("first_name", "middle_name", "last_name")):
                    updates["employee_name"] = " ".join(
                        part for part in [name_parts["first_name"], name_parts["middle_name"], name_parts["last_name"]] if _norm_text(part)
                    ).strip()
    ...
    return {
        ...
        "data": {
            "status": req.status,
            "review_state": final_workflow.get("review_state") or ("Applied" if req.status == "Applied" else "Pending Review"),
        },
    }
...
def claim_request(request_name: str, reviewer_email: str, release: bool = False) -> dict[str, Any]:
    ...
    if release:
        if existing_owner and existing_owner != reviewer_email:
            return {"success": False, "error": f"Request is claimed by {existing_owner}", "code": "CLAIM_CONFLICT"}
        workflow["claimed_by"] = ""
        workflow["claimed_at"] = ""
    else:
        if existing_owner and existing_owner != reviewer_email:
            return {"success": False, "error": f"Request is already claimed by {existing_owner}", "code": "CLAIM_CONFLICT"}
