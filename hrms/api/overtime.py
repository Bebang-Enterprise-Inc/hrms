...
from frappe.utils import flt, nowdate, now_datetime, getdate, add_days, cint
...
@frappe.whitelist()
def get_overtime_requests(status=None, store=None, from_date=None, to_date=None, page=1, page_size=20):
    """List overtime requests for the queue table with pagination."""
    _check_ot_approver_role()
    ...
    for row in rows:
        row.regular_hours = flt(row.regular_hours)
        row.overtime_hours = flt(row.overtime_hours)
...
def get_overtime_summary(store=None, from_date=None, to_date=None):
    """Return grouped overtime summary for analytics view."""
    _check_ot_approver_role()
    ...
    summary = frappe.db.sql(
        ...
        SELECT
            ...
            COALESCE(emp.branch, '') AS store,
            COUNT(ot.name) AS total_requests,
        ...
        LEFT JOIN `tabEmployee` emp ON emp.name = ot.employee
        WHERE {where}
        GROUP BY emp.department, emp.branch
        ORDER BY emp.department ASC, emp.branch ASC
        """,
        params,
        as_dict=True,
    )
    ...
    for row in summary:
        normalized_row = {
            ...
            "pending_hours": flt(row.pending_hours),
            "rejected_hours": flt(row.rejected_hours),
            "approved_count": cint(row.approved_count),
            "pending_count": cint(row.pending_count),
            "rejected_count": cint(row.rejected_count),
        }
        normalized.append(normalized_row)

        totals["total_requests"] += normalized_row["total_requests"]
        totals["total_hours"] += normalized_row["total_hours"]
        totals["approved_hours"] += normalized_row["approved_hours"]
        totals["pending_hours"] += normalized_row["pending_hours"]
        totals["rejected_hours"] += normalized_row["rejected_hours"]

    return {"summary": normalized, "totals": totals}
