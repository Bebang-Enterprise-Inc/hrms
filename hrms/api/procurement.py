...
def get_suppliers(filters=None, page=1, page_size=20, search=None):
	...
	if search:
		conditions.append("(supplier_code LIKE %(search)s OR supplier_name LIKE %(search)s)")
	...
	# Get total count
	total = frappe.db.sql(f"SELECT COUNT(*) FROM `tabBEI Supplier` WHERE {where_clause}", values)[0][0]
...
def create_supplier(data):
	...
	if contact_number:
		existing = frappe.db.get_value(
			"BEI Supplier", {"contact_number": contact_number}, ["name", "supplier_name"], as_dict=True
		)
		if existing:
			frappe.throw(
				_(
					"Phone number {0} is already used by supplier: {1} ({2}). "
					"Duplicate contact info may indicate shell companies."
				).format(contact_number, existing.supplier_name, existing.name),
	...
	if email:
		existing = frappe.db.get_value(
			"BEI Supplier", {"email": email}, ["name", "supplier_name"], as_dict=True
		)
		if existing:
			warnings.append(
				_("Email {0} is already used by supplier: {1}").format(email, existing.supplier_name)
			)
	...
	if bank_account:
		existing = frappe.db.get_value(
			"BEI Supplier", {"bank_account_number": bank_account}, ["name", "supplier_name"], as_dict=True
		)
		if existing:
			frappe.throw(
				_(
					"Bank account {0} is already used by supplier: {1} ({2}). "
					"This is a critical fraud indicator."
				).format(bank_account, existing.supplier_name, existing.name),
	...
	tin = data.get("tin")
	if tin:
		existing = frappe.db.get_value("BEI Supplier", {"tin": tin}, ["name", "supplier_name"], as_dict=True)
		if existing:
			frappe.throw(
				_(
					"TIN {0} is already registered to supplier: {1} ({2}). "
					"Each supplier must have a unique TIN."
				).format(tin, existing.supplier_name, existing.name),
				title=_("Duplicate TIN Detected"),
			)

	supplier = frappe.get_doc({"doctype": "BEI Supplier", **_sanitize_doc_data(data)})
	supplier.insert()
...
def get_purchase_requisitions(filters=None, page=1, page_size=20, search=None):
	...
	if search:
		conditions.append("(pr_no LIKE %(search)s OR department LIKE %(search)s)")
		values["search"] = f"%{search}%"
	...
	total = frappe.db.sql(f"SELECT COUNT(*) FROM `tabBEI Purchase Requisition` WHERE {where_clause}", values)[
		0
	][0]
...
def create_purchase_requisition(data=None):
	...
	pr = frappe.get_doc({"doctype": "BEI Purchase Requisition", **_sanitize_doc_data(data)})
	pr.insert()
...
def get_purchase_orders(filters=None, page=1, page_size=20, search=None):
	...
	if search:
		conditions.append("(po_no LIKE %(search)s OR supplier_name LIKE %(search)s)")
		values["search"] = f"%{search}%"
	...
	if filters:
		...
		if filters.get("pending_approval"):
			conditions.append("status IN ('Pending Mae Approval', 'Pending Butch Approval')")

	where_clause = " AND ".join(conditions) if conditions else "1=1"
	offset = (int(page) - 1) * int(page_size)

	total = frappe.db.sql(f"SELECT COUNT(*) FROM `tabBEI Purchase Order` WHERE {where_clause}", values)[0][0]
...
def create_purchase_order(data=None):
	...
	if supplier_name:
		...
		if flt(annual_purchases) + po_value > 250000:
			if not supplier.tin:
				frappe.throw(
					_(
						"Supplier {0} requires TIN registration. "
						"Annual purchases (₱{1:,.2f}) + this PO (₱{2:,.2f}) exceed ₱250,000 threshold. "
						"Update supplier master data before proceeding."
					).format(supplier.supplier_name, annual_purchases, po_value),
	...
	po = frappe.get_doc({"doctype": "BEI Purchase Order", **_sanitize_doc_data(data)})
	po.insert()
...
def get_goods_receipts(filters=None, page=1, page_size=20, search=None):
	...
	if search:
		conditions.append("(gr_no LIKE %(search)s OR supplier_name LIKE %(search)s)")
		values["search"] = f"%{search}%"
	...
	total = frappe.db.sql(f"SELECT COUNT(*) FROM `tabBEI Goods Receipt` WHERE {where_clause}", values)[0][0]
...
def create_goods_receipt(data=None):
	...
	if purchase_order:
		...
		if gr_date < po_date:
			frappe.throw(
				_("GR date ({0}) cannot be earlier than PO date ({1})").format(gr_date, po_date),
	...
	gr = frappe.get_doc({"doctype": "BEI Goods Receipt", **_sanitize_doc_data(data)})
	gr.insert()
...
def get_pending_gr_for_po(purchase_order):
	...
	return {"purchase_order": po.as_dict(), "pending_items": pending_items}
...
def get_invoices(filters=None, page=1, page_size=20, search=None):
	...
	total = frappe.db.sql(f"SELECT COUNT(*) FROM `tabBEI Invoice` WHERE {where_clause}", values)[0][0]
...
def create_invoice(data):
	...
	if purchase_order:
		...
		if not gr_exists:
			...
			if not approved_exception:
				return {
					...
					"message": _(
						"No Goods Receipt found for PO {0}. " "An exception approval is required to proceed."
		...
		if invoice_date < po_date:
			frappe.throw(
				_(
					"Invoice date ({0}) cannot be earlier than PO date ({1}). "
					"This indicates the invoice was created before the PO was approved."
				).format(invoice_date, po_date),
				title=_("Invalid Date Sequence"),
			)
		...
		if flt(po.grand_total) > 500000:
			if not po.mae_approval or not po.butch_approval:
				frappe.throw(
					_(
						"Cannot create Invoice: PO {0} (₱{1:,.2f}) requires complete approval "
						"(CPO + CFO) before invoicing"
					).format(po.po_no, po.grand_total),
					title=_("Approval Required"),
				)

	invoice = frappe.get_doc({"doctype": "BEI Invoice", **_sanitize_doc_data(data)})
	invoice.insert()
...
def get_payment_requests(filters=None, page=1, page_size=20, search=None):
	...
	if search:
		conditions.append("(pr.payment_request_no LIKE %(search)s OR pr.supplier_name LIKE %(search)s)")
		values["search"] = f"%{search}%"
	...
	total = frappe.db.sql(f"SELECT COUNT(*) FROM `tabBEI Payment Request` pr WHERE {where_clause}", values)[
		0
	][0]
...
def create_payment_request(data):
	...
	if _po_for_guard:
		...
		if total_advance_outstanding > 0:
			po_total = flt(frappe.db.get_value("BEI Purchase Order", _po_for_guard, "grand_total"))
			if total_advance_outstanding >= po_total:
				frappe.throw(
					_(
						"This PO is fully covered by an outstanding advance payment of PHP {0:,.2f}. "
						"Cannot create duplicate payment."
					).format(total_advance_outstanding),
				...
				frappe.msgprint(
					_(
						"Note: PO {0} has outstanding advance(s) of PHP {1:,.2f}. "
						"Remaining payable: PHP {2:,.2f}."
					).format(_po_for_guard, total_advance_outstanding, remaining),
					alert=True,
				)
		...
		if purchase_order:
			...
			if not gr_exists:
				...
				if not approved_exception:
					return {
						...
						"message": _(
							"No Goods Receipt found for PO {0}. "
							"An exception approval is required to proceed."
						).format(purchase_order),
					}
				...
				if payment_amount > received_value:
					frappe.throw(
						_(
							"Payment amount (₱{0:,.2f}) exceeds received value (₱{1:,.2f}). "
							"You can only pay for goods actually received."
						).format(payment_amount, received_value),
			...
			if flt(po.grand_total) > 500000:
				if not po.mae_approval or not po.butch_approval:
					frappe.throw(
						_(
							"Cannot create Payment Request: PO {0} (₱{1:,.2f}) requires complete "
							"approval (CPO + CFO) before payment"
						).format(po.po_no, po.grand_total),
						title=_("Approval Required"),
					)
	...
	request = frappe.get_doc({"doctype": "BEI Payment Request", **_sanitize_doc_data(data)})
	request.insert()
...
def get_aging_analysis():
	...
	aging = {"current": 0, "days_1_30": 0, "days_31_60": 0, "days_61_90": 0, "over_90": 0}
...
@frappe.whitelist()
def get_payment_disbursement_report(
	from_date=None, to_date=None, payment_mode=None, supplier=None, page=1, page_size=50
):
	...
	total = frappe.db.sql(
		f"""SELECT COUNT(*) FROM `tabBEI Payment Request` pr
            LEFT JOIN `tabBEI Invoice` inv ON pr.invoice = inv.name
            WHERE {where_clause}""",
		values,
	)[0][0]
...
@frappe.whitelist()
def get_three_way_match_report(
	from_date=None, to_date=None, supplier=None, variance_only=False, page=1, page_size=50
):
...
@frappe.whitelist()
def get_goods_receipt_log(
	from_date=None, to_date=None, status=None, supplier=None, discrepancy_only=False, page=1, page_size=50
):
	...
	where_clause = " AND ".join(conditions)
	discrepancy_filter = (
		"AND (gr.total_received_qty != gr.total_ordered_qty OR gr.receipt_date > po.delivery_date)"
		if discrepancy_only
		else ""
	)
	offset = (page - 1) * page_size
...
def get_single_source_suppliers():
	...
	return {
		"data": data,
		"supplier_summary": sorted(supplier_concentration.values(), key=lambda x: -x["total_value"]),
		"summary": {
...
def get_supplier_duplicates():
	...
	return {
		"data": sorted(
			duplicates, key=lambda x: {"CRITICAL": 0, "HIGH": 1, "MEDIUM": 2}.get(x["risk_level"], 3)
		),
		"summary": {
...
def get_supplier_data_quality():
	...
	for sup in suppliers:
		...
		if not sup.bir_2307:
			supplier_issues.append(
				{"field": "BIR 2307", "severity": "MEDIUM", "message": "BIR 2307 certificate missing"}
			)

		if not sup.business_permit:
			supplier_issues.append(
				{"field": "Business Permit", "severity": "MEDIUM", "message": "Business permit missing"}
			)

		# Check contact info
		if not sup.contact_number and not sup.email:
			supplier_issues.append(
				{"field": "Contact Info", "severity": "LOW", "message": "No contact phone or email"}
			)
			...
			issues.append(
				{
					...
					"max_severity": max(i["severity"] for i in supplier_issues) if supplier_issues else None,
				}
			)

	# Sort by severity and annual purchases
	severity_order = {"CRITICAL": 0, "HIGH": 1, "MEDIUM": 2, "LOW": 3}
	issues.sort(key=lambda x: (severity_order.get(x.get("max_severity"), 4), -x.get("annual_purchases", 0)))
...
def get_supplier_aging(supplier):
	...
	supplier_invoices = [
		inv
		for inv in aging["invoice_details"]
...
def get_billing_list(store=None, status=None, billing_period=None, page=1, page_size=20):
	...
	total = frappe.db.sql(f"SELECT COUNT(*) FROM `tabBEI Billing Schedule` WHERE {where_clause}", values)[0][
		0
	]
...
def _send_ceo_exception_notification(exception_doc):
	...
	try:
		...
		po_no = (
			frappe.db.get_value("BEI Purchase Order", exception_doc.purchase_order, "po_no")
			or exception_doc.purchase_order
		)
		supplier_name = (
			frappe.db.get_value("BEI Purchase Order", exception_doc.purchase_order, "supplier_name") or ""
		)
		...
		from hrms.utils.bei_config import SPACE_ERP_AUTOMATION, get_chat_space
		...
		frappe.log_error(
			...
			message=f"Exception: {exception_doc.name}, Error: {e!s}",
		)
		return False
	...
	if reference_type == "BEI Distribution Trip":
		...
		if existing:
			...
			return {
				...
				"message": _("An exception request already exists for Trip {0} stop {1}: {2}").format(
...
def upload_official_receipt(name, or_number, or_date, or_amount, or_attachment=None):
	...
	if pay_req.status not in ("Paid - Awaiting OR", "Paid"):
		frappe.throw(
			_("Cannot upload OR for payment with status '{0}'. " "Expected 'Paid - Awaiting OR'.").format(
				pay_req.status
			)
		)
...
def get_awaiting_or_list(page=1, page_size=20, status=None, supplier=None, overdue_only=False):
	...
	total = frappe.db.sql(f"SELECT COUNT(*) FROM `tabBEI Payment Request` pr WHERE {where}", params)[0][0]
...
def mark_or_not_required(name, reason=None):
	...
	if pay_req.status not in ("Paid - Awaiting OR", "Paid"):
		frappe.throw(_("Cannot modify OR status for payment with status '{0}'").format(pay_req.status))
	...
	frappe.get_doc(
		{
			...
			"reference_name": name,
			"content": _("OR marked as not required by {0}. Reason: {1}").format(frappe.session.user, reason),
		}
...
def send_or_follow_up(name):
	...
	if pay_req.status != "Paid - Awaiting OR":
		frappe.throw(_("Cannot send follow-up for payment with status '{0}'").format(pay_req.status))
...
def check_overdue_or():
	...
	for pay in overdue_payments:
		...
		if pay.or_status != "Overdue":
			frappe.db.set_value(
				"BEI Payment Request", pay.name, "or_status", "Overdue", update_modified=False
			)

		# Escalation tiers
		if days_overdue >= 30:
			# Escalate to CFO (Butch)
			_create_or_escalation_notification(pay, "CFO", "butch@bebang.ph", days_overdue)
		elif days_overdue >= 14:
			# Escalate to CPO (Mae)
			_create_or_escalation_notification(pay, "CPO", "mae@bebang.ph", days_overdue)
...
def _create_or_escalation_notification(payment, role_label, recipient, days_overdue):
	...
	try:
		...
		frappe.log_error(
			f"Failed to create OR escalation notification for {payment.name}", "OR Escalation Error"
		)
...
def tag_advance_to_gr(advance_payment, goods_receipt, amount_to_clear):
	...
	try:
		...
		frappe.msgprint(_("Advance clearing JV created: {0}").format(jv.name), indicator="green")
...
def mark_advance_undeliverable(advance_payment, amount, reason):
	...
	try:
		...
		jv.company = get_company()
		jv.user_remark = (
			f"Reclassification of undeliverable advance: {advance_payment}. Reason: {reason.strip()}"
		)
		jv.append(
			"accounts",
			{
		...
		frappe.get_doc(
			{
				...
				"content": f"Reclassified PHP {amount:,.2f} to AR-Others (JV: {jv.name}). Reason: {reason.strip()}",
			}
		).insert(ignore_permissions=True)
...
def get_advance_subsidiary_ledger(supplier, from_date=None, to_date=None):
	...
	journal_entries = frappe.db.sql(
        ...
        ORDER BY je.posting_date""".format(
			date_filter=je_date_filter.replace("posting_date", "je.posting_date")
		),
		values,
		as_dict=True,
	)
...
def get_form_2307_data(supplier=None, tax_period=None):
	...
	summary = {}
	for entry in entries:
		key = "{}|{}".format(entry.get("supplier_name", ""), entry.get("tax_period", ""))
...
def apply_franchise_payment(
	...
	try:
		...
		if billing.status not in ("Sent", "Partially Paid"):
			frappe.throw(_("Cannot apply payment to billing with status '{0}'").format(billing.status))

		# Save payment_proof attachment if provided
		if payment_proof:
			from hrms.api.store import save_base64_image

			file_url = save_base64_image(payment_proof, "BEI Billing Schedule", billing.name, "payment_proof")
