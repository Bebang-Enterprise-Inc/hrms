...
def approve_mrf(mrf_name, action, notes=None):
	...
	if current_status == "Pending Hiring Manager":
		...
		if not any(r in roles for r in ["HR Manager", "System Manager"]):
			...
			dept_head = frappe.db.get_value("Department", mrf.department, "department_head")
			if current_emp != dept_head:
				frappe.throw(
					_("Only the department head of {0} can approve at this stage").format(mrf.department)
				)
	...
	if action == "reject":
		...
		if current_status == "Pending Hiring Manager":
			...
			ceo_approval_required = any(
				keyword in designation.upper() for keyword in ["VP", "VICE PRESIDENT", "C-LEVEL", "DIRECTOR"]
			)
...
def get_recruitment_pipeline(status=None, department=None):
	...
	if department:
		job_openings = frappe.get_all("Job Opening", filters={"department": department}, pluck="name")
...
def get_recruitment_metrics(from_date=None, to_date=None):
	...
	return {
		...
		"mrf": {
			...
			"approval_rate": round((mrf_approved / mrf_created * 100), 2) if mrf_created > 0 else 0,
		},
		"openings": {
			"created": openings_created,
			"filled": openings_filled,
			"fill_rate": round((openings_filled / openings_created * 100), 2) if openings_created > 0 else 0,
		},
		"applicants": {
