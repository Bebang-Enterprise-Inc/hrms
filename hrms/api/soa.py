...
def generate_soa(store, period):
    ...
    existing = frappe.db.exists("BEI Statement of Account", {
        "store": store,
        "soa_period": period,
    ...
    monthly_billings = frappe.get_all("BEI Billing Schedule",
        filters={
            "store": store,
        ...
        fields=["name", "billing_type", "total_amount", "generated_on",
                "royalty_fee", "management_fee", "marketing_fee", "ecommerce_fee"]
    )

    # Delivery billings: match by date range (generated_on)
    delivery_billings = frappe.get_all("BEI Billing Schedule",
        filters={
            "store": store,
        ...
        fields=["name", "billing_type", "total_amount", "generated_on",
                "delivery_fee", "logistics_fee", "goods_value", "handling_fee",
                "cargo_type", "trip_reference"]
    )
...
def get_soa_list(store=None, period=None, status=None):
    ...
    return frappe.get_all("BEI Statement of Account",
        filters=filters,
        fields=["name", "store", "store_type", "soa_period", "status",
                "total_billings", "total_payments", "balance_due",
                "generated_on", "sent_on"],
...
def get_soa_detail(soa_name):
    """Get SOA header + line-items detail for a single record."""
    if not soa_name:
