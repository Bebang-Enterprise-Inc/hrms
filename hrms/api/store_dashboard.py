...
def get_pending_reports(
	...
	if sort == "expected_by":
		pending.sort(
			key=lambda item: (item.get("expected_by", ""), item.get("store", ""), item.get("report_type", ""))
		)
	elif sort == "report_type":
		pending.sort(
			key=lambda item: (item.get("report_type", ""), item.get("store", ""), item.get("expected_by", ""))
		)
	else:
		pending.sort(
			key=lambda item: (item.get("store", ""), item.get("report_type", ""), item.get("expected_by", ""))
		)
