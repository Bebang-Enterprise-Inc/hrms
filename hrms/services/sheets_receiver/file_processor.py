...
class FileProcessor:
	...
	def process_queued_file(self, queue_entry: dict[str, Any]) -> dict[str, Any]:
		...
		try:
			...
			self.db.update_file_status(
				file_id, "completed", record_count=result["record_count"], report_type=result["report_type"]
			)
			...
			self.db.update_file_status(file_id, "failed", error=error_msg)
	...
	def process_pending_files(self, limit: int = 10, parallel: bool = True) -> dict[str, Any]:
		...
		if not pending:
			return {"status": "no_pending_files", "processed": 0, "failed": 0}
		...
		if parallel and len(pending) > 1:
			...
			with ThreadPoolExecutor(max_workers=workers) as executor:
				# Submit all tasks
				future_to_entry = {
					executor.submit(self.process_queued_file, entry): entry for entry in pending
				}
	...
	def process_all_pending(self, batch_size: int = 25) -> dict[str, Any]:
		...
		total_results = {"status": "completed", "processed": 0, "failed": 0, "total_records": 0, "batches": 0}
