...
from .config import get_config, SheetConfig, get_watched_sheets
...
class SheetsClient:
    ...
    def create_watch(self, spreadsheet_id: str, spreadsheet_name: str) -> WatchChannel:
        ...
        try:
            ...
            logger.info(f"Created watch for {spreadsheet_name} ({spreadsheet_id}), "
    ...
    def setup_all_watches(self) -> List[WatchChannel]:
        ...
        for key, sheet_config in get_watched_sheets().items():
            ...
            if existing and not existing.is_expired and existing.hours_until_expiry > 2:
                logger.info(f"Watch for {sheet_config.name} still valid "
    ...
    def renew_expiring_watches(self, hours_before: int = 2) -> List[WatchChannel]:
        ...
        for old_watch in expiring:
            try:
                ...
                # Create new watch
                new_watch = self.create_watch(
                    old_watch.spreadsheet_id,
                    old_watch.spreadsheet_name
                )
