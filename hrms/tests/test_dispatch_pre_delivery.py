...
def _install_fake_frappe_and_dependencies():
    ...
    if "hrms.api.procurement" not in sys.modules:
        ...
        procurement_mod.request_match_exception = lambda payload: {"success": True, "name": "BEI-EXC-0001", "payload": payload}
...
class TestDispatchPreDelivery(unittest.TestCase):
    ...
    def test_request_pre_delivery_billing_exception_calls_procurement_api(self):
        with patch("hrms.api.procurement.request_match_exception", return_value={"success": True, "name": "BEI-EXC-0092"}) as request_match:
    ...
    def test_create_pre_delivery_billing_calls_internal_guarded_path(self):
        stop = _Stop(idx=1, status="Pending", billing_reference="BILL-0001", billing_creation_status="Success")
        trip = _Trip(stop)
