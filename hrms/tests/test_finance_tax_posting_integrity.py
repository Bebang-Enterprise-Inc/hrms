...
class TestFinanceTaxPostingIntegrity(unittest.TestCase):
	...
	def test_billing_vat_is_split_only_from_vat_inclusive_fees(self):
		...
		self.assertAlmostEqual(
			accounts["4000003 - Royalty Income - BEI"]["credit_in_account_currency"], 100.0, places=2
		)
		self.assertAlmostEqual(
			accounts["4000004 - Management Fee Income - BEI"]["credit_in_account_currency"], 50.0, places=2
		)
		self.assertAlmostEqual(
			accounts["4000006 - Marketing Income - BEI"]["credit_in_account_currency"], 80.0, places=2
		)
		self.assertAlmostEqual(
			accounts["2102205 - Output VAT Payable - BEI"]["credit_in_account_currency"], 18.0, places=2
		)
		self.assertAlmostEqual(
			accounts["1103101 - Accounts Receivable - BEI"]["debit_in_account_currency"], 248.0, places=2
		)
	...
	def test_advance_payment_applies_ewt_without_any_vat_row(self):
		...
		with (
			patch.object(
				doc, "_get_payment_accounts", return_value=("1100 - Cash and Bank - BEI", "Wire Transfer")
			),
