...
class FakeMRF:
	def __init__(
		self, name="MRF-0001", status="Draft", department="Operations", designation="Store Supervisor"
	):
		self.name = name
		self.status = status
...
class RecruitmentNotificationTests(unittest.TestCase):
	...
	def test_create_mrf_emits_transition_notification(self):
		...
		self.assertTrue(
			any(
				"Draft" in (text or "") and "Pending Hiring Manager" in (text or "")
				for _, text in mrf.comments
			)
		)
	...
	def test_approve_mrf_emits_pending_hr_manager_notification(self):
		...
		self.assertTrue(
			any(
				"Pending Hiring Manager" in (text or "") and "Pending HR Manager" in (text or "")
				for _, text in mrf.comments
			)
		)
		recruitment.frappe.publish_realtime.assert_called_once()
