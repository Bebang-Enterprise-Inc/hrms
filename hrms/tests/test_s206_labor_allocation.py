...
class BuildPairedJEsTests(unittest.TestCase):
    ...
    def _call(self, amount=9000.0, share=0.3):
        def fake_resolve(company):
            if "BEI" in company or "ENTERPRISE" in company:
                return self.mock_home_accts
            return self.mock_covered_accts
        with patch.object(labor_allocation, "_resolve_company_accounts", side_effect=fake_resolve):
    ...
    def test_home_je_due_from_row_is_debit_with_company_party(self):
        home, _covered = self._call(amount=9000.0)
        ...
        self.assertEqual(due_from_row["party_type"], "Company")
        self.assertEqual(due_from_row["party"], "SM MEGAMALL - BEBANG ENTERPRISE INC.")
    ...
    def test_covered_je_due_to_row_is_credit_with_company_party(self):
        _home, covered = self._call(amount=9000.0)
        ...
        self.assertEqual(due_to_row["party_type"], "Company")
        # party = counterparty (home Company)
        self.assertEqual(due_to_row["party"], "BEBANG ENTERPRISE INC.")
