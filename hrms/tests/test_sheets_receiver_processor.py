...
class TestSheetsReceiverProcessorCriticalAlerts(unittest.TestCase):
	...
	def test_sync_sheet_splits_large_inventory_chunks_with_distinct_checksums(self):
		processor = self._make_processor()
		sheet_config = types.SimpleNamespace(
			name="Inventory",
			spreadsheet_id="sheet-003",
			...
			related_sheet_keys=[],
			sync_chunk_field="warehouse_source_code",
			sync_chunk_max_rows=2,
		)
		rows = [
			{"inventory_key": "3MD::PM001", "item_code": "PM001", "warehouse_source_code": "3MD", "qty": 73.0},
			{"inventory_key": "3MD::RM015", "item_code": "RM015", "warehouse_source_code": "3MD", "qty": 18.0},
			{"inventory_key": "3MD::CM30", "item_code": "CM30", "warehouse_source_code": "3MD", "qty": 4.0},
		]
		processor.sheets.fetch_sheet_data = MagicMock(return_value=(rows, "chk-inventory"))
		processor.frappe.sync_sheet_data = MagicMock(
			side_effect=[
		...
		first_call = processor.frappe.sync_sheet_data.call_args_list[0]
		second_call = processor.frappe.sync_sheet_data.call_args_list[1]
		self.assertEqual(len(first_call.args[1]), 2)
		self.assertEqual(len(second_call.args[1]), 1)
		self.assertNotEqual(first_call.args[2], second_call.args[2])
