...
const COL_NAMES = ['SOURCE', 'PAYEE', 'INVOICE NO.', 'INVOICE DATE', 'AMOUNT', 'OUTSTANDING',
                   ...
                   'CHECK NO.', 'CATEGORY', 'CLASSIFICATION', 'BILLED TO', 'VATABLE', 'VAT', 'EWT'];
...
function seedNewInvoicesFromSources_(ss, fpmLookup, taxLookup, dryRun) {
  ...
  hoData.slice(2).forEach((r, i) => {
    ...
    if (!payee) return;
    const classification = (r[hi('CLASSIFICATION')] || '').toString().trim().toUpperCase();
...
function buildGrid(rows, label, ts, isCapex, isVatGaps) {
  ...
  hdr.push(['SOURCE', 'PAYEE', 'INVOICE NO.', 'INVOICE DATE', 'AMOUNT', 'OUTSTANDING', 'AGING', 'AGING BUCKET', 'STATUS', 'BEI-FIN No.', 'RFP No.', 'METHOD', 'CHECK NO.', 'CATEGORY', 'CLASSIFICATION', 'BILLED TO', 'VATABLE', 'VAT', 'EWT']);
