"""
BEI Brain S023A Phase 1A: Execute DDL via Supabase Management API.
Usage: python scripts/brain/execute_ddl.py [--dry-run]
"""
import sys
import subprocess
import json
...
-- AUDIT-3: updated_at trigger function
...
-- company_data: read-only for authenticated
...
-- frappe_events: read-only for authenticated
...
def verify_schema():
    ...
    checks = [
        ...
        ("memories table", "SELECT tablename FROM pg_tables WHERE schemaname = 'public' AND tablename = 'memories'"),
        ("company_data table", "SELECT tablename FROM pg_tables WHERE schemaname = 'public' AND tablename = 'company_data'"),
        ("frappe_events table", "SELECT tablename FROM pg_tables WHERE schemaname = 'public' AND tablename = 'frappe_events'"),
        ("brain_audit_log table", "SELECT tablename FROM pg_tables WHERE schemaname = 'public' AND tablename = 'brain_audit_log'"),
        ("unique constraint", "SELECT conname FROM pg_constraint WHERE conname = 'uq_company_data_entity'"),
        ("memories indexes", "SELECT indexname FROM pg_indexes WHERE tablename = 'memories' AND schemaname = 'public'"),
        ("frappe_events indexes", "SELECT indexname FROM pg_indexes WHERE tablename = 'frappe_events' AND schemaname = 'public'"),
        ("RLS enabled", "SELECT tablename FROM pg_tables WHERE schemaname = 'public' AND tablename IN ('memories','company_data','frappe_events') AND rowsecurity = true"),
    ]
    headers = {
        "Authorization": f"Bearer {TOKEN}",
        "Content-Type": "application/json",
    ...
    for label, sql in checks:
        resp = requests.post(API_URL, json={"query": sql}, headers=headers, timeout=30)
        if resp.status_code in (200, 201):
            data = resp.json()
            ...
            if status == "EMPTY":
                all_pass = False
            print(f"  {status}: {label} ({rows} rows)")
...
def main():
    ...
    if dry_run:
        print("=== DRY RUN ===")
        full_ddl = f"BEGIN;\n{DDL}\n{INDEXES_DDL}\n{VECTOR_INDEXES_DDL}\n{TRIGGERS_DDL}\nROLLBACK;"
        ok = run_sql(full_ddl, "Full DDL dry-run (tables + indexes + triggers)")
        if ok:
            print("\nDRY RUN PASSED")
        else:
            print("\nDRY RUN FAILED")
        ...
        steps = [
            ...
            (INDEXES_DDL, "Performance indexes"),
            (VECTOR_INDEXES_DDL, "HNSW vector indexes"),
            (TRIGGERS_DDL, "updated_at triggers"),
            (RLS_DDL, "Row Level Security policies"),
            (AUDIT_TRIGGERS_DDL, "Audit logging triggers"),
        ]
        for sql, label in steps:
            if not run_sql(sql, label):
                print(f"\nFAILED at: {label}")
                print("Schema partially deployed. Fix errors and re-run.")
                sys.exit(1)
