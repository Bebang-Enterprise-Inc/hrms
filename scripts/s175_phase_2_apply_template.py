#!/usr/bin/env python3
"""S175 Phase 2 — Apply MASTER_SALES_TEMPLATE to BKI + BEI + BFC.

Critical discovery: BKI and BEI have `parent_company='Triple I Holdings'` in
Frappe's Group Company tree. ERPNext's `validate_root_company_and_sync_account_to_children`
throws "Please add the account to root level Company - Triple I Holdings"
unless:
  1. TIH has `allow_account_creation_against_child_company=1`, OR
  2. The same-named account already exists on TIH, OR
  3. We set `frappe.local.flags.ignore_root_company_validation = True`

We use option 3 (Frappe-sanctioned per-request flag) to bypass the validator
for S175 template inserts. BFC has `parent_company=None` (standalone), so
inserts work directly.

COA corruption fixups also applied:
- BKI: 4000000 SALES has self-parent or was deleted earlier; recreate cleanly.
- BEI: 4000000 SALES was is_group=0 (posting); convert via SQL UPDATE.
"""
...
def main():
    ...
    payload_path = OUT / "_phase2_payload.py"
    payload_path.write_text(payload, encoding="utf-8")

    stdout, stderr, status = run_on_frappe(payload_path, tag="phase2_apply_template", timeout_seconds=1200)
    if status != "Success":
        print(f"SSM status={status}")
        print(stderr[-3000:])
        sys.exit(1)

    raw = stdout.split("S175_PHASE2_JSON_START", 1)[1].split("S175_PHASE2_JSON_END", 1)[0].strip()
    result = json.loads(raw)

    (OUT / "phase2_verification.json").write_text(json.dumps(result, indent=2, default=str), encoding="utf-8")

    if result.get("fixups"):
        print("Corruption fixups applied:")
        for f in result["fixups"]:
            print(f"  {f}")

    any_errors = False
    for comp, rep in result["summary"].items():
        if isinstance(rep, dict) and rep.get("errors"):
            any_errors = True
            print(f"PHASE2 ERRORS on {comp}:")
            for e in rep["errors"][:10]:
                print(f"  {e}")

    for comp in ["Bebang Kitchen Inc.", "Bebang Enterprise Inc.", "BEBANG FRANCHISE CORP."]:
        rep = result["summary"].get(comp, {})
        verif = result["verification"].get(comp, [])
        print(f"  {comp}: created={len(rep.get('created', []))} matched={len(rep.get('matched', []))} "
              f"reparented={len(rep.get('reparented', []))} root_type_fixed={len(rep.get('root_type_fixed', []))} "
              f"name_fixed={len(rep.get('name_fixed', []))} post_4000xxx={len(verif)}")

    if any_errors:
        sys.exit(2)

    # Assert minimums
    for comp, min_expected in [("Bebang Kitchen Inc.", 27),
                                ("BEBANG FRANCHISE CORP.", 27),
                                ("Bebang Enterprise Inc.", 28)]:
        actual = len(result["verification"][comp])
        if actual < min_expected:
            print(f"INCOMPLETE: {comp} has {actual} 4000xxx (expected >={min_expected})")
            sys.exit(3)
