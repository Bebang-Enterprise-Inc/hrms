#!/usr/bin/env python3
"""S222 final cleanup probe — find and cancel any lingering S222-window SEs + reset MRs.

Adapted from s218_final_cleanup_probe.py. S222 sweep ran ~2026-04-24 11:09 UTC → 13:15 UTC.
Uses creation window 2026-04-24 10:00:00 UTC to capture the full sweep.
"""
...
ses = frappe.db.sql("""
    ...
    WHERE se.creation >= '2026-04-24 10:00:00'
      AND se.docstatus = 1
""", as_dict=True)
print(f"Dangling S222 SEs (docstatus=1): {len(ses)}")
for se in ses:
...
def main():
    ...
    cmds = [
        ...
        f"echo '{enc}' | base64 -d > /tmp/s222_final.py",
        "docker cp /tmp/s222_final.py $BACKEND:/tmp/s222_final.py",
        "docker exec $BACKEND /home/frappe/frappe-bench/env/bin/python /tmp/s222_final.py",
    ]
