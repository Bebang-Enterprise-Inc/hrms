...
SCRIPT = HERE / "probe_cc_state.py"
OUT = HERE / "cc_probe_result.json"
ssm = boto3.client("ssm", region_name=REGION)
encoded = base64.b64encode(SCRIPT.read_text(encoding="utf-8").encode()).decode()
cmds = [
    "set -e",
    "BACKEND=$(docker ps --filter name=frappe_backend --format '{{.ID}}' | head -1)",
    f"echo '{encoded}' | base64 -d > /tmp/s247_cc.py",
    "docker cp /tmp/s247_cc.py $BACKEND:/tmp/s247_cc.py",
    "docker exec $BACKEND bash -lc 'cd /home/frappe/frappe-bench && env/bin/python /tmp/s247_cc.py'",
    "docker cp $BACKEND:/tmp/s247_cc_probe.json /tmp/s247_cc_probe.json",
    "gzip -c /tmp/s247_cc_probe.json > /tmp/s247_cc_probe.json.gz",
    "echo S247_FILE_BEGIN",
    "base64 -w0 /tmp/s247_cc_probe.json.gz; echo",
    "echo S247_FILE_END",
]
...
if "S247_FILE_BEGIN" in out_text:
