#!/usr/bin/env python3
"""SSM wrapper for cleanup_historical_test_bki_si.py."""
...
SCRIPT = HERE / "cleanup_historical_test_bki_si.py"
OUT = Path(__file__).parent.parent.parent / "output/l3/s247/verification/phase6_cleanup_result.json"
...
cmds = [
    ...
    f"echo '{encoded}' | base64 -d > /tmp/s247_p6.py",
    "docker cp /tmp/s247_p6.py $BACKEND:/tmp/s247_p6.py",
    "docker exec $BACKEND bash -lc 'cd /home/frappe/frappe-bench && env/bin/python /tmp/s247_p6.py'",
    "docker cp $BACKEND:/tmp/s247_p6_cleanup.json /tmp/s247_p6_cleanup.json",
    "gzip -c /tmp/s247_p6_cleanup.json > /tmp/s247_p6_cleanup.json.gz",
    "echo S247_FILE_BEGIN",
    "base64 -w0 /tmp/s247_p6_cleanup.json.gz; echo",
    "echo S247_FILE_END",
]
