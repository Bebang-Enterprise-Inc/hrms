		...
		for i, punch in enumerate(punches):
			...
			if status == 200:
				...
				if "already" in message.lower() or "Duplicate" in message:
					...
					skipped += 1
					skipped_details.append(f"  SKIP: pin={pin} time={event_time} reason=no_employee")
					last_successful_time = event_time
				else:
					# Unknown validation error — log and continue
					skipped += 1
					skipped_details.append(f"  SKIP: pin={pin} time={event_time} reason={message[:100]}")
					last_successful_time = event_time
...
if __name__ == "__main__":
	...
	parser.add_argument(
		"--batch-size", type=int, default=BATCH_SIZE, help=f"Max punches per run (default: {BATCH_SIZE})"
	)
