...
def upsert_to_supabase(location_id: int, date_str: str, weather_data: Dict) -> bool:
    ...
    weather_description = WEATHER_CODES.get(
        weather_data['weather_code'],
        f"Unknown ({weather_data['weather_code']})"
    )
    ...
    rain_hours = 8 if weather_data['precipitation'] > 5 else (4 if weather_data['precipitation'] > 1 else (1 if weather_data['precipitation'] > 0 else 0))
...
def sync_weather(date_str: str, warehouse_tree_path: str) -> Dict:
    ...
    for group_idx, group in enumerate(groups, 1):
        ...
        seed = group[0]
        print(f"[Group {group_idx}/{len(groups)}] Fetching weather at ({seed['latitude']:.4f}, {seed['longitude']:.4f})")
        ...
        if not weather_data:
            print(f"   [WARN] Failed to fetch weather data, skipping group")
        ...
        print(f"   Temperature: {weather_data['temperature_mean']:.1f}C (max: {weather_data['temperature_max']:.1f}C)")
