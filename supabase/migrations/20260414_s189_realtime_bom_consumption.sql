...
-- IDEMPOTENT: safe to re-run. Tracked by scripts/apply_migration.py via
-- public.schema_migrations with checksum verification.
...
CREATE INDEX IF NOT EXISTS idx_product_bom_product ON public.product_bom(product_name);
CREATE INDEX IF NOT EXISTS idx_product_bom_material ON public.product_bom(material_code);
...
CREATE TABLE IF NOT EXISTS public.daily_material_consumption (
    id BIGSERIAL PRIMARY KEY,
...
-- Idempotent unique constraint (wrapped in DO block to check existence)
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint
        WHERE conname = 'idx_dmc_unique'
          AND conrelid = 'public.daily_material_consumption'::regclass
    ) THEN
...
CREATE INDEX IF NOT EXISTS idx_dmc_date ON public.daily_material_consumption(business_date);
CREATE INDEX IF NOT EXISTS idx_dmc_material ON public.daily_material_consumption(material_code);
CREATE INDEX IF NOT EXISTS idx_dmc_date_material ON public.daily_material_consumption(business_date, material_code);
CREATE INDEX IF NOT EXISTS idx_dmc_location ON public.daily_material_consumption(location_id);
...
-- T1.3: Attach triggers (idempotent via DROP IF EXISTS)
...
CREATE TABLE IF NOT EXISTS public.daily_material_consumption_frappe_sync (
    id BIGSERIAL PRIMARY KEY,
    business_date DATE NOT NULL,
    material_code TEXT NOT NULL,
