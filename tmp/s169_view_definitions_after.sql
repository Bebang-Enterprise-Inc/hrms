-- === MV: public.discount_summary === (subs=1)
CREATE MATERIALIZED VIEW public.discount_summary AS
 SELECT s.store_name,
   ...
   FROM (((pos_order_items i
     JOIN v_pos_orders_live o ON ((o.id = i.order_id)))
...
-- === MV: public.daily_revenue === (subs=1)
...
-- === MV: public.store_daily_closing === (subs=12)
 ...
 SELECT s.location_id,
   ...
   FROM (stores s
     JOIN ( SELECT all_orders.location_id,
           ...
           FROM ( SELECT v_pos_orders_live.location_id,
                    v_pos_orders_live.business_date,
                    'pos'::text AS source,
                    count(*) AS order_count,
                    sum(v_pos_orders_live.original_gross_sales) AS orig_gross,
                    sum(v_pos_orders_live.gross_sales) AS after_disc,
                    sum(v_pos_orders_live.net_sales) AS net_vat,
                    sum(v_pos_orders_live.total_discounts) AS discounts,
                    sum(v_pos_orders_live.vat_amount) AS vat,
                    sum(v_pos_orders_live.vat_exempt_sales) AS vat_exempt,
                    (0)::numeric AS del_fee
                   FROM v_pos_orders_live
                  WHERE (v_pos_orders_live.payment_status = 'PAID'::text)
                  GROUP BY v_pos_orders_live.location_id, v_pos_orders_live.business_date
                UNION ALL
...
-- === MV: public.sales_dashboard_daily_store_metrics === (subs=9)
CREATE MATERIALIZED VIEW public.sales_dashboard_daily_store_metrics AS
 WITH pos_sales AS (
         SELECT v_pos_orders_live.location_id,
            v_pos_orders_live.business_date,
            (count(*))::integer AS pos_orders,
            (COALESCE(sum(v_pos_orders_live.gross_sales), (0)::numeric))::numeric(14,2) AS pos_gross_sales,
            (COALESCE(sum(v_pos_orders_live.net_sales), (0)::numeric))::numeric(14,2) AS pos_net_sales_without_vat
           FROM v_pos_orders_live
          WHERE (v_pos_orders_live.payment_status = 'PAID'::text)
          GROUP BY v_pos_orders_live.location_id, v_pos_orders_live.business_date
        ), web_sales AS (
...
-- === MV: public.payment_reconciliation === (subs=1)
CREATE MATERIALIZED VIEW public.payment_reconciliation AS
 SELECT s.store_name,
   ...
   FROM ((pos_order_payments p
     JOIN v_pos_orders_live o ON ((o.id = p.order_id)))
     JOIN stores s ON ((s.location_id = o.location_id)))
  WHERE (o.payment_status = 'PAID'::text)
  GROUP BY s.store_name, o.business_date, p.payment_type;

-- === MV: public.store_daily_baselines === (subs=7)
CREATE MATERIALIZED VIEW public.store_daily_baselines AS
 SELECT location_id,
   ...
   FROM ( SELECT v_pos_orders_live.location_id,
            v_pos_orders_live.business_date AS order_date,
            count(*) AS order_count
           FROM v_pos_orders_live
          WHERE ((v_pos_orders_live.business_date >= (CURRENT_DATE - '56 days'::interval)) AND (v_pos_orders_live.payment_status = 'PAID'::text))
          GROUP BY v_pos_orders_live.location_id, v_pos_orders_live.business_date) daily
  GROUP BY location_id, (EXTRACT(dow FROM order_date));

-- === VIEW: public.v_all_channel_daily === (subs=9)
 ...
 SELECT dates.business_date,
    ...
    concat_ws('+'::text,
        ...
        CASE
            ...
            ELSE NULL::text
        END) AS data_sources
   FROM (((( SELECT DISTINCT v_pos_orders_live.business_date
           FROM v_pos_orders_live
          WHERE (v_pos_orders_live.payment_status = 'PAID'::text)
        UNION
     ...
     LEFT JOIN ( SELECT v_pos_orders_live.business_date,
            count(*) AS order_count,
            sum(v_pos_orders_live.gross_sales) AS gross_sales,
            sum(v_pos_orders_live.net_sales) AS net_sales
           FROM v_pos_orders_live
          WHERE (v_pos_orders_live.payment_status = 'PAID'::text)
          GROUP BY v_pos_orders_live.business_date) pos USING (business_date))
...
-- === VIEW: public.v_ops_weekly === (subs=0)
...
-- === VIEW: public.v_system_daily_totals === (subs=0)
...
-- === VIEW: public.v_monthly_store_summary === (subs=1)
...
-- === VIEW: public.v_discount_identity_order_usage === (subs=1)
 ...
 WITH normalized_source AS (
         SELECT po.business_date,
           ...
           FROM ((pos_order_items poi
             JOIN v_pos_orders_live po ON ((po.id = poi.order_id)))
...
-- === VIEW: public.v_discount_identity_rolling_30d_usage === (subs=0)
WITH base AS (
...
-- === MV: public.payment_reconciliation ===
CREATE MATERIALIZED VIEW public.payment_reconciliation AS
 SELECT s.store_name,
    o.business_date,
    p.payment_type,
...
-- === MV: public.store_daily_baselines ===
CREATE MATERIALIZED VIEW public.store_daily_baselines AS
 SELECT location_id,
    EXTRACT(dow FROM order_date) AS day_of_week,
    percentile_cont((0.5)::double precision) WITHIN GROUP (ORDER BY ((order_count)::double precision)) AS median_orders,
...
-- === MV: public.sales_dashboard_daily_store_metrics ===
CREATE MATERIALIZED VIEW public.sales_dashboard_daily_store_metrics AS
 WITH pos_sales AS (
...
-- === VIEW: public.v_system_daily_totals ===
CREATE OR REPLACE VIEW public.v_system_daily_totals AS
 SELECT business_date,
    count(*) AS store_count,
    sum(total_orders) AS total_orders,
